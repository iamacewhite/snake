######################################################################
# 		COMP 2611 Game: Greedy Snake 			     #		                             
# 	Author: BAI Chunyan  Student ID:20253689		     #						     
...
#score variable
score: 		.word 0
#stores how many points are recieved for eating a block
#increases as program gets harder
score_for_one:	.word 10
#speed the snake moves, increases as game progresses
gameSpeed:	.word 200
#array to store the scores in which difficulty should increase
scoreThresholds: .word 100, 250, 500, 1000, 5000, 10000
# to calculate which difficulty is met
scoreArrayPosition: .word 0
#end game message
lostMessage:	.asciiz "You have lost:( Your score was: "
...
#this is used to have the tail to follow the head correctly
...
#Draw Background to Black, for reset
	lw $a0, screenWidth #screen width
	lw $a1, backgroundColor #background color
...
DrawTailUp:
	...
	jal DrawPixel	
	j Drawblock  #finished updating snake, update block
...
Drawblock:
	...
	beq $v0, 1, AddLength #if block was eaten, add length

	#draw the block
	lw $a0, blockPositionX
	lw $a1, blockPositionY
	jal CoordinateToAddress
	add $a0, $v0, $zero
	lw $a1, blockColor
	jal DrawPixel
...
IncreaseDifficulty:
	...
	la $t1, scoreThresholds #get the milestones base address
	...
	#load the score_for_one variable to increase the
	#points awarded for eating block
	lw $t0, score_for_one
	#multiply gain by 2
	sll $t0, $t0, 1 
	#load the game speed
	lw $t1, gameSpeed
	#add 25 to the move speed
	addiu $t1, $t1, 25
	#store new speed
	sw $t1, gameSpeed
